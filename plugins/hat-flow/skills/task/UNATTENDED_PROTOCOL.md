...
**激活时机：** P2 Step 2e 配置面板（主要入口）或 Phase 1/2/3 完成后（后备入口，仅当 unattended.json 尚不存在时）。
...
| 批准 design.md（Step 8） | reviewer 通过（Critical=0 且 Important=0）后自动批准 |
...
|---|---|
| Light 验证未配置 | 跳过验证 |
| BLOCKED | 重试一次（Opus），仍失败 → auto-cancel + Telegram |
